        ...
        <?php
            ...
            if($docente==true){
                ...
                echo '<div id="navbar" class="z-depth-2">';
                    ...
                    echo '<a href="pag2.php" class="buttonlinks"><div id="navbut"><div id="nav1txt"><center>Tue domande</center></div></div></a>';
                echo '</div>';
                        echo '<div id="middle">';
                    $db=mysqli_connect("localhost","root","") or die("Errore connessione: ".mysqli_error());
                    mysqli_select_db($db,"faq");
                    $ris=mysqli_query($db,"select * from Domande join Utenti using(IDUtenteDom) where IDUtenteDom='$id' order by DataDom desc, OraDom desc;") or die('<div id="container" class="z-depth-1">Errore nella query: '.mysqli_error($db).'</div>');
                    if(mysqli_num_rows($ris)==0){
                        echo '<div id="container" class="z-depth-1"><div style="padding: 20px 20px 20px 20px; font-family: Roboto-Regular; font-size: 20px; color: rgba(0,0,0,0.87);">Nessuna domanda fatta da te!</div></div>';
                        ...
                        while($riga){
                            ...
                            echo '<div id="container" class="z-depth-1">';
                                ...
                                echo '<div class="rcont">';
                                    ...
                                    if(mysqli_num_rows($ris2)==0){
                                        ...
                                        while($riga2){
                                            ...
                                            $query=mysqli_query($db,'select N_stelle,IDVotazione from Votazioni where IDUtenteVotazione="'.$id.'" and IDRisposta="'."$IDRisposta".'";') or die("Errore query: ".mysqli_error());
                                            if(mysqli_num_rows($query)==0){
                                                ...
                                                for($i=0;$i<5;$i++){
                                                    ...
                                                    echo '<input type="hidden" name="IDUtenteVot" value="'.$id.'">';
                                ...
                                echo '</div>';
                                echo '<div class="divider"></div>';
                                    $query3=mysqli_query($db,"select * from Risposte join Utenti on Risposte.IDUtenteRisp=Utenti.IDUtenteDom where Risposte.IDDomanda='$IDDomanda' and Risposte.IDUtenteRisp='".$id."' and Utenti.prof=0;") or die('Errore nella query: '.mysqli_error($db));
                                    if(mysqli_num_rows($query3)==0){
                                        ...
                                        echo '<div id="bottt">';
                                        echo '<input type="hidden" name="IDDom" value="'.$IDDomanda.'">';
                                        echo '<input type="hidden" name="IDUtenteRisp" value="'.$id.'">';
                    ...
                    echo '<div id="container" class="z-depth-1">';
                        echo '<div class="ncont">';
                            echo '<form id="nuovad" action="script/nuovad.php" method="post">';
                                ...
                                echo '<br>';
                                echo '<br>';
                                echo '<textarea rows="1" onkeyup="textAreaAdjust(this);" id="ndom" class="textfield" name="testo" maxlength="400" placeholder="Descrizione nuova domanda (facoltativa)"></textarea>';
                                echo '<br>';
                                echo '<br>';
                                echo '<div id="bottt">';
                                    echo '<input type="hidden" name="IDUtenteDom" value="'.$id.'">';
